...
class GIID_Net(nn.Module):
    def __init__(self):
        super(GIID_Net, self).__init__()
...
# Used only for training one-shot NAS
class GIID_Net_NAS(nn.Module):
    def __init__(self):
        super(GIID_Net_NAS, self).__init__()
...
class GIID_Model(nn.Module):
    def __init__(self):
        ...
        self.lr = 1e-4
        self.networks = GIID_Net()
        # self.networks = GIID_Net_NAS()
        ...
        with open('log.txt', 'a+') as f:
            f.write('\n\nGIID-Net, Total Params: %d' % pytorch_total_params)
...
class InpaintingForensics():
    def __init__(self):
        ...
        test_dataset = GIID_Dataset(self.test_num, self.test_file, choice='test')
